...
def _prepare_transaction(txn):
    ...
    txn.amount = round(max(float(txn.amount), 100001.0), 2)
    return txn
...
def _create_producer(settings) -> Producer:
    config = {
        ...
        "client.id": "credit-card-stream-simulator-fraud",
        "acks": "all",
        "retries": 5,
...
def main() -> None:
    ...
    try:
        logger.info("Starting fraud producer for topic=%s", settings.topic_name)
        if not running:
            return
        ...
        # validate_json_payload only proves the dict can be encoded, not that
        # it has the fields bronze/silver expect.
        try:
